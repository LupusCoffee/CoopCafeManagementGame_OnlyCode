...
void UHolderComponent::PickUpItem(UHeldItem* ItemToPickUp)
{
	...
	if (UPrimitiveComponent* PrimitiveComponent = HeldItem->GetPrimitiveComponent())
	{
		Multicast_UpdatePrimitiveComponentPhysicsAndCollision(PrimitiveComponent, HeldItem->InitiallyHadPhysics(),
			false, ECollisionEnabled::NoCollision);

		//if held item root is different from primitive comp, attach primitive to root - why?
		if (HeldItemActor->GetRootComponent() != PrimitiveComponent)
		{
			PrimitiveComponent->AttachToComponent(
				HeldItemActor->GetRootComponent(),
...
void UHolderComponent::DropItem(FVector InLookedAtLocation)
{
	...
	{
		Multicast_UpdatePrimitiveComponentPhysicsAndCollision(PrimitiveComponent, HeldItem->InitiallyHadPhysics(),
			true, ECollisionEnabled::QueryAndPhysics);
...
void UHolderComponent::DetachItemStillAsHeld()
{
	if (!HeldItem) return;
...
void UHolderComponent::ThrowItem(AController* _Controller, float ThrowForce)
{
	...
	//turn on physics
	Multicast_UpdatePrimitiveComponentPhysicsAndCollision(PrimitiveComponent, HeldItem->InitiallyHadPhysics(), true, ECollisionEnabled::QueryAndPhysics);
...
AActor* UHolderComponent::GetHeldItemAsActor()
{
	if (!HeldItem) return nullptr;
	return HeldItem->GetActor();
}
...
bool UHolderComponent::IsHoldingActor(AActor* ActorToCheck)
{
	if (!ActorToCheck) return false;
	return GetHeldItemAsActor() == ActorToCheck;
}


//Replication
// multicast
void UHolderComponent::Multicast_UpdatePrimitiveComponentPhysicsAndCollision_Implementation(
	UPrimitiveComponent* PrimitiveComponent, bool bHadPhysics, bool bPhysicsOn, ECollisionEnabled::Type Col)
{
	if (!PrimitiveComponent) return;

	if (bPhysicsOn) //when turning ON physics, collision should be set first
	{
		// Update collision regardless whether they had physics or not
		PrimitiveComponent->SetCollisionEnabled(Col);
		
		if (bHadPhysics) PrimitiveComponent->SetSimulatePhysics(bPhysicsOn);
	}
	else			//when turning OFF physics, physics should be set first
	{
		if (bHadPhysics) PrimitiveComponent->SetSimulatePhysics(bPhysicsOn);
